multiJob('xray_demo/xray_demo_DocMgmts') {
		    displayName('XRay Demo DocMgmts')
		    description('Folder for XRAY Demo Analysis of current txmutual artifacts')
}

job('xray_demo/xray_demo_DocMgmts/XRay Demo - eStampApp') {

  	  properties {
        githubProjectUrl('file:///opt/XRayDemo/DocMgmts/eStampApp')
    }
  
    scm {
    	git {
          remote {
	  	       url('file:///opt/XRayDemo/DocMgmts/eStampApp')
          }
          branch('**/master')
          extensions {
                    wipeOutWorkspace()
