multiJob('xray_demo/xray_demo_UWApps') {
		    displayName('XRay Demo UWApps')
		    description('Folder for XRAY Demo Analysis of current txmutual artifacts')
}

job('xray_demo/xray_demo_UWApps/XRay Demo - InterimAuditReporting') {

  	  properties {
        githubProjectUrl('file:///opt/XRayDemo/UWApps/InterimAuditReporting')
    }
  
    scm {
    	git {
          remote {
	  	       url('file:///opt/XRayDemo/UWApps/InterimAuditReporting')
          }
          branch('**/master')
          extensions {
                    wipeOutWorkspace()
