...
job('nexus_iq_demo/nexus_iq_demo_UWApps/XRay Demo - MasterAgentCommission')
{
    scm {
        git {
        remote {
                    url('file:///opt/XRayDemo/UWApps/MasterAgentCommission')
        }
        branch('**/master')
            extensions {
                        wipeOutWorkspace()
    ...
    steps {
        iqPolicyEvaluatorBuildStep {
          ...
          iqApplication {
            manualApplication {
              applicationId('MasterAgentCommission2')
            }
          }
